...
func buildQueries(req *plugin.GenerateRequest, options *opts.Options, structs []Struct) ([]Query, error) {
	...
	for _, query := range req.Queries {
		...
		if len(query.Params) == 1 && qpl != 0 {
			...
			var cols []goColumn
			idColumnBatch := []int{}
			if query.Cmd == metadata.CmdDeleteBatch {
				regexCompile := regexp.MustCompile(`([\(]?\s*\?\s*[\)]?)`) // in (?), ?
				matches := regexCompile.FindAllStringSubmatch(query.Text, -1)
				for id, m := range matches {
					if strings.Contains(m[1], "(") {
						idColumnBatch = append(idColumnBatch, id)
					}
				}
			}
